...
from VictorAI import AutoencoderKL, DDIMScheduler, DDPMScheduler, StableVictorPipeline, UNet2DConditionModel
...
def parse_args(input_args=None):
    ...
    parser.add_argument("--save_interval", type=int, default=500, help="Save weights every N steps.")
    ...
    #For Queries.txt file 
    parser.add_argument("--current_query", type=str, help="Current query for training")
    parser.add_argument("--folder_location", type=str, default="./upscaled_output", help="Location to store downloaded images")
...
class DreamBoothDataset(Dataset):
    def __init__(
        self,
        current_query,  #current_query as an argument
        tokenizer,
        ...
        self.current_query = current_query  # Save current_query as an instance variable

        self.instance_images_path = []
        self.class_images_path = []

        upscaled_output_path = Path("./upscaled_output/")

        # Get all folders inside the specified directory
        folders = [folder for folder in upscaled_output_path.iterdir() if folder.is_dir()]

        for folder in folders:
          
        # Check if the folder name matches the current query
            if folder.name == self.current_query:
                inst_img_path = [
                    (x, folder.name)  # Use folder name as the prompt for instance images
                    for x in folder.iterdir()
                    if x.is_file() and not x.name.endswith((".txt", ".csv"))
                ]
                ...
                if with_prior_preservation:
                    class_img_path = [
                        (x, folder.name)  # Use folder name as the prompt for class images
                        for x in folder.iterdir()
                        if x.is_file() and not x.name.endswith((".txt", ".csv"))
                    ]
...
# Main function for the DreamBooth pipeline
def main_dreambooth(args):
# def main(args):
    ...
    # Load models and create wrapper for diffusion
