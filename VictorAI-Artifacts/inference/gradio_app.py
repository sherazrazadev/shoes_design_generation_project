...
def run_pipeline(prompt, negative_prompt, num_samples, guidance_scale, num_inference_steps, height, width, strength, image_path):
    # Set up arguments
    args = argparse.Namespace(
        ...
        # Set other arguments to their default values here
        model_path="victorai_shoe_model/1000",
        output_folder='model_generated_output',
        project_name='Project',
        yolo_model='./best.pt',
        real_esrgan_weights="shoes_classification_localization_with_High_Resolution/RealESRGAN_x4.pth",
        sam_checkpoint_path='tmp/sam_vit_h_4b8939.pth'
    )
    ...
    # Fourth Model - 3D Zereo123
    segmented_images = Image_3d_converter(output_folder_sr, args)

    print("Segmented images",segmented_images)
    
    # Check if segmented_images is None
    if segmented_images is None:
        print("No segmented images were returned.")
        return []

    # Assuming segmented_images is a list of file paths to the generated images
    generated_images = []
...
iface = gr.Interface(
    fn=run_pipeline,
    inputs=[
        gr.Textbox(label="Prompt", placeholder="Enter a prompt for image generation"),
        gr.Textbox(label="Negative Prompt", placeholder="Enter a negative prompt"),
        gr.Slider(1, 10, label="Number of Samples", value=1),
        gr.Slider(1, 10, label="Guidance Scale", value=7),
        gr.Slider(10, 100, label="Number of Inference Steps", value=50),
        gr.Slider(256, 1024, label="Height", value=512),
        gr.Slider(256, 1024, label="Width", value=512),
        gr.Slider(0, 1, label="Strength Scale", value=1),
